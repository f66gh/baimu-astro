...
- 学术成果和个人笔记的 `date` 表示初始发布日期；最后更新日期自动读取该文章文件最近一次 Git 提交时间，按北京时间显示，无需手填。未提交的新文章或没有 Git 历史时，回退到 `date`。
- 这两个板块保留置顶优先，组内按最后更新日期降序排列。修改后需要提交并重新构建，线上日期才会更新；图片、公共组件等其他文件的修改不会更新文章日期。
- `npm run build` 会先检查 Git 历史；若部署环境使用浅克隆（只下载近期提交），会自动拉取完整历史，以便读取旧文章的更新时间。
